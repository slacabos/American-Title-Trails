...
### 9. Add Inns and Cathedrals (Expansion)

If you want to match the first Carcassonne expansion:

- **Inns on roads:** Double points if completed, 0 if incomplete
- **Cathedrals in cities:** Triple points if completed, 0 if incomplete
- Adds risk/reward strategy
...
1. ✅ Fix majority rule calculation
2. ✅ Remove 2-tile minimum for cities
3. ✅ Add claim validation

### Phase 2: Major Feature (3-5 days)

4. ✅ Implement field/farmer mechanics
   - Define field segments on tiles
   - Detect field boundaries
   - Track adjacent cities
   - Score at game end

### Phase 3: Polish (1-2 days)

5. ✅ Update AI to use new mechanics
...
describe("Majority Rule", () => {
  it("should award points to player with most followers", () => {
    // Player 1: 2 followers, Player 2: 1 follower
    // Player 1 should get all points
  });

  it("should award points to all players in a tie", () => {
    // Player 1: 2 followers, Player 2: 2 followers
    // Both should get full points
  });
});

describe("Field Scoring", () => {
  it("should score farmers for adjacent completed Costcos", () => {
    // Place farmer on field
    // Complete 2 Costco shopping areas adjacent to that field
    // Farmer should score 6 points (2 Costcos × 3 points)
  });

  it("should separate fields by roads", () => {
    // Straight road tile creates 2 separate fields (left and right)
    // Farmer on left field should NOT score for Costcos touching right field
    // Should be scored independently
  });

  it("should separate fields by Costcos", () => {
    // Large Costco divides field areas
    // Farmers on different sides should score independently
  });

  it("should apply majority rule to farmers", () => {
    // Field with Player A having 2 farmers, Player B having 1 farmer
    // Field touches 2 completed Costcos
    // Only Player A should score (has majority)
  });
});

describe("Costco Completion", () => {
  it("should complete single-tile Costcos", () => {
    // Place tile with Costco on one edge only, other edges are fields
    // Should complete immediately and score 2 points
  });

  it("should complete single-tile corner Costcos", () => {
    // Place tile with Costco on two adjacent edges (corner), other edges are fields
    // Should complete immediately and score 2 points
  });
});

describe("Feature Claiming", () => {
  it("should prevent claiming already-claimed features", () => {
    // Attempt to place follower on feature that has one
    // Should be rejected
  });

  it("should allow merging claimed and unclaimed features", () => {
    // Place tile that connects claimed road to unclaimed road
    // Should merge into single feature with one follower
  });
});
```
...
Your implementation is **architecturally sound** with good separation of concerns and clean code. The main gaps are:

1. **Field scoring** (biggest missing feature)
2. **Majority rule** (affects competitive balance)
3. **Single-tile city completion** (minor scoring issue)
4. **Claim validation** (prevents illegal moves)

Implementing these will bring your game to full Carcassonne parity. The good news is your architecture supports these additions without major refactoring.
