...
function Review() {
  ...
  return (
    <main style={{ font: "16px Arial, sans-serif", color: "#30443b", width: 1100 }}>
      <h1>{example.name}</h1>
      <p>
        <span data-testid="sections">{count}</span> tiles · <span data-testid="complexes">{layout.length}</span>{" "}
        connected complexes
      </p>
      <button disabled={count === 1} onClick={() => setCount(count - 1)}>
        Remove tile
      </button>
      <button disabled={count === example.records.length} onClick={() => setCount(count + 1)}>
        Add tile
      </button>
      <div style={{ height: 650, background: "#eee9dd" }}>
        ...
        >
          ...
          {records.flatMap((record) =>
            record.tile.costcoZones.map((_, i) => {
              const [x, z] = featureAnchor(record.tile, { type: "costco", identifier: `costco_${i}` });
              return (
                <Follower
                  key={`${record.position.x},${record.position.y}:${i}`}
                  point={[record.position.x + x, 0.012, record.position.y + z]}
                  color="#437eaf"
                />
              );
            }),
