...
function placeNext(game: Game) {
  for (let turns = 0; turns < 4; turns++) {
    ...
    if (position) {
      expect(game.placeTile(position).success).toBe(true);
      return;
    }
...
describe("River opening", () => {
  ...
  it.each(RIVER_TILES)(
    "rotates and renders $id with matching water portals and dry building footprints",
    (definition) => {
      ...
      for (let turns = 0; turns < 4; turns++) {
        ...
        for (const edge of tile.river!.edges)
          expect(rotated.getEdge(DIRECTIONS[(DIRECTIONS.indexOf(edge) + turns) % 4])).toBe("river");
        const path = riverPath(rotated);
        expect(path.length).toBeGreaterThan(2);
        for (const edge of rotated.river!.edges)
          expect(
            path.some((point) => Math.hypot(point[0] - PORTALS[edge][0], point[1] - PORTALS[edge][1]) < 1e-8),
          ).toBe(true);
  ...
  it("counts only Costcos touching the farmer's bank and distinguishes features on the same tiles", () => {
    ...
    const cap = new Tile({
      ...
      roadConnections: [],
      costcoZones: [{ id: "cap", segments: ["south"] }],
      fieldSegments: [],
    });
  ...
  it("traces both zones when opposite-bank Costcos reconnect around the end of a river", () => {
    ...
    const dual = byId("river-dual-costco");
    board.tiles.set("0,0", { tile: dual, position: { x: 0, y: 0 } });
    const cap = (id: string, segments: (typeof DIRECTIONS)[number][]) =>
      new Tile({
        id,
        name: id,
        center: "costco",
  ...
  it("scores a shared incomplete bridge road once per majority holder", () => {
    const board = new Board();
    const end = buildDeck()
      .find((tile) => tile.id === "road-end")!
      .rotate(1);
    const straight = buildDeck()
      .find((tile) => tile.id === "straight-road")!
      .rotate(1);
  ...
  it.each([false, true])(
    "scores only after claiming and returns completed followers (already claimed: %s)",
    (alreadyClaimed) => {
      const game = new Game(players, { seed: 41 });
      for (let i = 0; i < 10; i++) {
        placeNext(game);
        ...
        const decision = new StrategicAI().evaluateMeeplePlacement(
          {
            board,
            currentTile: end,
            currentPlayer: { ...current, followers: 1 },
            allPlayers: state.players,
            validPlacements: [],
            gameState: game.getState(),
  ...
  it.each(["easy", "medium", "hard", "expert"] as AIDifficulty[])(
    "finishes seeded games with %s AI without losing river tiles",
    (difficulty) => {
      for (let seed = 1; seed <= 8; seed++) {
        const game = new Game(
          players.map((p) => ({ ...p, isAI: true, aiDifficulty: difficulty })),
          { seed },
        );
        let steps = 0;
