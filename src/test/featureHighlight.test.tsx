  ...
  BoardView: ({ state, highlightedFeature, onTilePlace }: ComponentProps<typeof BoardView>) => {
    ...
    return (
      <div>
        ...
        <button disabled={!position} onClick={() => onTilePlace(position)}>
          Place test tile
        </button>
      </div>
    );
...
function placeTile() {
  for (
    let rotation = 0;
    rotation < 4 && screen.getByRole("button", { name: "Place test tile" }).hasAttribute("disabled");
    rotation++
  ) {
...
describe("claim highlight lifetime", () => {
  it.each(["claim", "skip"])(
    "clears a selected Costco when %s removes the controls without blur or mouseleave",
    (action) => {
