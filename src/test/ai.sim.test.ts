...
const gamesPerPairOverride = Number.parseInt(process.env.AI_SIM_GAMES_PER_PAIR ?? "", 10);
const GAMES_PER_PAIR =
  Number.isFinite(gamesPerPairOverride) && gamesPerPairOverride > 0 ? gamesPerPairOverride : DEFAULT_GAMES_PER_PAIR;
const MAX_TURNS = 10000;
const BASE_SEED = 1337;
const minHighWinOverride = Number.parseFloat(process.env.AI_SIM_MIN_HIGH_WIN_RATE ?? "");
const maxLowWinOverride = Number.parseFloat(process.env.AI_SIM_MAX_LOW_WIN_RATE ?? "");
const MIN_HIGH_WIN_RATE = Number.isFinite(minHighWinOverride) && minHighWinOverride >= 0 ? minHighWinOverride : 0.6;
const MAX_LOW_WIN_RATE = Number.isFinite(maxLowWinOverride) && maxLowWinOverride >= 0 ? maxLowWinOverride : 0.4;
...
const playGame = (
  ...
  if (!game.getState().isGameOver) {
    throw new Error(`Game did not finish in ${MAX_TURNS} turns (${difficultyA} vs ${difficultyB})`);
  }
...
describe("AI difficulty balance (simulation)", () => {
  it("should favor higher difficulties across all pairings", { timeout: TEST_TIMEOUT_MS }, async () => {
    ...
    for (const [pairIndex, [low, high]] of pairings.entries()) {
      ...
      const passed = highWinRate >= MIN_HIGH_WIN_RATE && lowWinRate <= MAX_LOW_WIN_RATE;
      ...
      expect(
        highWinRate,
        `${high} vs ${low} highWinRate=${highWinRate.toFixed(3)} ties=${ties}`,
      ).toBeGreaterThanOrEqual(MIN_HIGH_WIN_RATE);
      expect(lowWinRate, `${high} vs ${low} lowWinRate=${lowWinRate.toFixed(3)} ties=${ties}`).toBeLessThanOrEqual(
        MAX_LOW_WIN_RATE,
      );
    }
