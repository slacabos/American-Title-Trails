...
const tiles = [...new Map([getStartTile(), ...buildDeck()].map((tile) => [tile.id, tile])).values()];
...
describe("3D tile layout", () => {
  ...
  it.each(tiles.map((tile) => [tile.id, tile] as const))(
    ...
    (_, tile) => {
      for (let turns = 0; turns < 4; turns++) {
        ...
        expect(canonicalTile(rotated).roadConnections).toEqual(tile.roadConnections);
        ...
        rotated.roadConnections.forEach((road) => {
          ...
          expect(path[path.length - 1]).toEqual(PORTALS[endpoints[1] ?? "center"]);
        });
        rotated.costcoZones.forEach((zone, index) => {
          const polygon = zonePolygon(rotated, index);
          for (const segment of zone.segments.filter((part) => part !== "center")) {
  ...
  it("keeps separate outlets distinct and doesn't create a shared paved center", () => {
    ...
    expect(featureAnchor(tile, { type: "costco", identifier: "costco_0" })[1]).toBeLessThan(0);
    expect(featureAnchor(tile, { type: "costco", identifier: "costco_1" })[1]).toBeGreaterThan(0);
  });
    ...
    for (const tile of tiles)
      for (let turns = 0; turns < 4; turns++) {
        ...
        const features = [
          ...
          ...(rotated.hasMcDonalds ? [{ type: "mcdonalds" as const, identifier: undefined }] : []),
        ];
        for (const feature of features) {
          const board = new Board();
          board.placeTile(rotated, position);
          const claim = board.claimFeature(feature.type, position, feature.identifier, "p1");
  ...
  it("places farmers on a corner within their field instead of an obstructed centroid", () => {
    ...
    expect(featureAnchor(tile, { type: "field", identifier: "field_0" })).toEqual(CORNERS.nw);
    expect(featureAnchor(tile.rotate(1), { type: "field", identifier: "field_0" })).toEqual(CORNERS.ne);
  });
  ...
  it("updates legal placement and claim snapshots despite a stable board reference", () => {
    ...
    while (!boardSnapshot(game.getState()).legal.length) game.rotateTileClockwise();
    ...
    expect(boardSnapshot({ ...after, phase: GamePhase.PLACE_TILE, isGameOver: true }).legal).toEqual([]);
    const ai = {
      ...before,
