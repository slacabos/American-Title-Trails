...
describe('SimpleAI', () => {
  ...
  beforeEach(() => {
    ...
    aiPlayer = new Player('AI Player', { id: 'ai', isAI: true, color: '#0000FF' });
  ...
  describe('planMove', () => {
      ...
      if (validPlacements.length > 0) {
        ...
        const mockGame = {
          board: {
            ...
            canPlaceTile: vi.fn().mockReturnValue({ success: true, completedFeatures: [] }),
        ...
        expect(move?.position).toMatchObject({ x: expect.any(Number), y: expect.any(Number) });
    ...
    it('should prefer moves that complete features', () => {
      ...
      const mockGame = {
        board: {
          getPlacementCandidates: vi.fn().mockReturnValue([position1, position2]),
          getNeighbors: vi.fn().mockReturnValue({}),
          canPlaceTile: vi.fn()
            .mockReturnValueOnce({ success: true, completedFeatures: [{ type: 'road', points: 4 }] }) // Complete feature
