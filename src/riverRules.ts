...
export function riverPlacementError(
  ...
  const open = river.flatMap((record) =>
    record.tile.river!.edges.flatMap((edge) => {
  ...
  if (position.x !== head.next.x || position.y !== head.next.y || !tile.river.edges.includes(incoming))
    return "riverContinue";
